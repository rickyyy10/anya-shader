/*
    Anya pixel-art mesh ray tracer for Shadertoy
    ---------------------------------------------
    ...
    Artistic pass: explicit sRGB handling, distance-aware mip selection,
    pixel-grid rays, stepped lighting, Bayer dithering and silhouette accents.
...
float orderedDither(vec2 pixelCoordinate)
{
    ivec2 cell = ivec2(mod(floor(pixelCoordinate), 4.0));
    int index = cell.x + cell.y * 4;
    return (BAYER_4X4[index] + 0.5) * (1.0 / 16.0) - 0.5;
}
...
vec3 shadeMesh(vec3 point, vec3 rayDirection, Hit hit)
{
    ...
    // Shadertoy Custom Texures supplies the atlas as raw sRGB bytes. Decode it
    // explicitly so the local preview and Shadertoy use identical color math.
    // A ray-footprint approximation selects lower mips at a distance, removing
    // the high-frequency hair moire without derivatives crossing atlas islands.
...
#if PIXEL_ART_MODE
    diffuse = floor(diffuse * 5.0 + 0.5) * 0.2;
    wrap = floor(wrap * 5.0 + 0.5) * 0.2;
#endif
    ...
    vec3 lighting = vec3(0.42, 0.44, 0.48)
                  + vec3(1.00, 0.92, 0.84) * diffuse * 0.78
                  + vec3(0.35, 0.43, 0.58) * fill * 0.16
                  + vec3(0.18, 0.20, 0.24) * wrap * 0.06;
    vec3 color = albedo * lighting;
    color += vec3(1.0, 0.92, 0.82) * specular * 0.12;
    color += vec3(0.25, 0.38, 0.62) * rim * 0.05;

#if PIXEL_ART_MODE
    float silhouette = smoothstep(0.64, 0.88,
        1.0 - max(dot(normal, viewDirection), 0.0));
    color *= mix(vec3(1.0), vec3(0.20, 0.16, 0.24), silhouette * 0.72);
#endif
...
{
    vec2 sampleCoordinate = fragCoord;

#if PIXEL_ART_MODE
    sampleCoordinate = (floor(fragCoord / PIXEL_SIZE) + 0.5) * PIXEL_SIZE;
#endif

    vec2 uv = (2.0 * sampleCoordinate - iResolution.xy) / iResolution.y;
    ...
    color = linearToSrgb(acesTonemap(color * 1.16));

#if PIXEL_ART_MODE
    float dither = orderedDither(floor(fragCoord / PIXEL_SIZE));
    color = clamp(floor(color * COLOR_LEVELS + 0.5 + dither * 0.52)
        / COLOR_LEVELS, 0.0, 1.0);
#endif
